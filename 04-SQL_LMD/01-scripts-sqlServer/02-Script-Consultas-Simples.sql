...
GO
 ...
 --a)trabajan en Daimiel, Navarra, Castellón
...
SELECT Ciudad
FROM Oficinas
 WHERE Ciudad= 'Daimiel, Navarra, Castellón';

 SELECT *
FROM Representantes
WHERE Oficina_Rep IN ('Daimiel', 'Navarra', 'Castellón')
    OR
 ...
 (R.Ventas>Cuota AND R.Ventas<=600000);
	...
	--el numero de pedidos, su importe, el nombre y limite de credito del cliente


	SELECT 
    P.Num_Pedido AS NumeroPedido,
    P.Importe,
    C.Empresa,
    C.Limite_Credito
FROM Pedidos P
INNER JOIN Clientes C ON P.Cliente= C.Empresa;
